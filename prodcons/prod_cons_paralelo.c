 ...
 * Rafael Timbo Matos
 * RA106228
 * Produtor e Consumidor Paralelo
 *
 */
#include <stdio.h>
#include <omp.h>
...
void producer_consumer(int *buffer, int size, int *vec, int n, int nthreads) {
	int i, j;
...
#pragma omp parallel num_threads(nthreads) \
  default(none) shared(n, vec, size, buffer) private(i, j) reduction(+:sum)
	for(i=0;i<n;i++) {
		if(i % 2 == 0) {	// PRODUTOR
#pragma omp for
			for(j=0;j<size;j++) {
...
int main(void) {
  ...
  producer_consumer(buffer, size, vec, n, nthreads);
